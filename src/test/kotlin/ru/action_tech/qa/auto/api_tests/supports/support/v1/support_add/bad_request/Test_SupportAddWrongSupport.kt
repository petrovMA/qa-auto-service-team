...
class Test_SupportAddWrongSupport {
    ...
    @DisplayName("/api/v1/support-add -> 400 Bad Request: Метод создания сопровождения клиента (supportType не передан)")
    @AllureId("145906")
    fun test() {
        checkBR(
            apiClient = supportsCrmClient,
            request = request,
            expected = setOf(BrokenRule(code = 4, message = "Не указан тип сопровождения."))
        )
    }
}