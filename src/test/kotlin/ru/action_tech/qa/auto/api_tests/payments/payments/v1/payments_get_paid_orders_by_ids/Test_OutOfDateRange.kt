...
class Test_OutOfDateRange {
    private lateinit var orderId: String
    private lateinit var paymentIds: List<String>

    private val firstPaymentDate: LocalDate by lazy { TODAY.minusDays(2) }
    private val lastPaymentDate: LocalDate by lazy { TODAY.plusDays(2) }

    private val testCases by lazy {
        mapOf(
            "from = день последней оплаты + 1" to Pair(
                getStringTime(lastPaymentDate.plusDays(1), DateTimeFormatter.ISO_LOCAL_DATE),
                null
            ),
            "from в прошлом и to = день первой оплаты - 1" to Pair(
                getStringTime(firstPaymentDate.minusDays(4), DateTimeFormatter.ISO_LOCAL_DATE),
                getStringTime(firstPaymentDate.minusDays(1), DateTimeFormatter.ISO_LOCAL_DATE)
            ),
            "from = день первой оплаты + 1 и to = день последней оплаты - 1" to Pair(
    ...
    fun precondition() {
        ...
        paymentIds = "Создаем оплаты и сохраняем идентификаторы" {
            setOf(firstPaymentDate, lastPaymentDate)
                .map {
    ...
    @Test
    @DisplayName("$paymentsGetPaidOrdersByIds -> 200 Ok: out of payments range")
    @AllureId("245223")
    fun test() {
        testCases.forEach { (testName, dates) ->
            testName {
                ...
                ).apply { "Проверяем что массив в ответе пустой" { assertTrue(isEmpty()) } }
            }
        }
    }
}