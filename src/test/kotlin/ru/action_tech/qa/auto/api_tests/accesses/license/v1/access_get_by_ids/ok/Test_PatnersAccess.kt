...
class Test_PatnersAccess {
    ...
    @DisplayName("/api/v1/access_get-by-ids -> 200 ok: Партнерский доступ")
    @AllureId("220368")
    fun test() {
        accessesCrmClient.send(
            AccessesRequests.accessGetByIds(
                ...
                accessIds = arrayOf(
                    "32CB9010-3920-4D94-A8D9-266E8119524E"
                )
            )
        ).first().apply {
            ...
            "Проверяем, что параметр partnerSubscribeId не null" {
                assertThat(this.partnerSubscribeId).isNotNull()
            }
        }
    }
}